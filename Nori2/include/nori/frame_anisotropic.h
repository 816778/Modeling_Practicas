...
struct Frame_Anisotropic {
    ...
    Frame_Anisotropic(const Vector3f &s, const Vector3f &t, const Normal3f &n)
     : s(s), t(t), n(n) { }

    /// Construct a frame from the given orthonormal vectors
    Frame_Anisotropic(const Vector3f &x, const Vector3f &y, const Vector3f &z)
     : s(x), t(y), n(z) {}

    /// Construct a new coordinate frame from a single vector
    Frame_Anisotropic(const Vector3f &n) : n(n) {
    ...
    static float sinPhi(const Vector3f &v) {
        float sinTheta = Frame_Anisotropic::sinTheta(v);
        if (sinTheta == 0.0f)
            return 1.0f;
    ...
    /// Equality test
    bool operator==(const Frame_Anisotropic &frame) const {
